...
public class ElsblockProcessor extends Thread {
	...
	private boolean controlLeftRight=true;
	private int col = 4;
	private int row = -1;
	...
	public int  checkFullLine() {
		...
		for (int i =0; i<GameCanvas.CANVAS_ROWS ; i++) {//这里要注意，要从上往下判断，如果从下往上，就无法连消		
			...
			if (fullLine) {				
				...
				ElsblockProcessor.score=ElsblockProcessor.score+PER_LINE_SCORE;
				ElsblockProcessor.scoreForLevelUpdata=ElsblockProcessor.scoreForLevelUpdata+PER_LINE_SCORE;
				if(ElsblockProcessor.scoreForLevelUpdata>=PER_LEVEL_SCORE){
